...
from mdescriptor.descriptors.model_backed._vendor.dpa4desc.dpmodel.utils import (
    PairExcludeMask,
)
from mdescriptor.descriptors.model_backed._vendor.dpa4desc.dpmodel.utils.network import (
    NativeLayer,
)
from mdescriptor.descriptors.model_backed._vendor.dpa4desc.dpmodel.utils.seed import (
    child_seed,
)
from mdescriptor.descriptors.model_backed._vendor.dpa4desc.utils.charge_state import (
...
if TYPE_CHECKING:
    ...
    from mdescriptor.descriptors.model_backed._vendor.dpa4desc.utils.data_system import (
        DeepmdDataSystem,
    )
    from mdescriptor.descriptors.model_backed._vendor.dpa4desc.utils.path import (
        DPPath,
    )
...
class DescrptDPA4C(NativeOP, BaseDescriptor):
    ...
    def call(
        ...
        envelope
            ...
            ``(F, N_local, N_slot, 1)``.
        """
        from mdescriptor.descriptors.model_backed._vendor.dpa4desc.dpmodel.utils.neighbor_graph import (
    ...
    def build_invariant_descriptor(
        ...
        # MDescriptor exposes calibration as an explicit inference option.
        # Keep the official checkpoint statistics intact by default, while
        # allowing a caller that requests raw features to bypass both the
        # affine calibration and the spin gate below.
        if not getattr(self, "_calibrate_output", True):
            return descriptor
