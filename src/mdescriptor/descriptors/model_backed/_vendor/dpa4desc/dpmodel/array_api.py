...
def xp_sigmoid(x: Array) -> Array:
    ...
    if array_api_compat.is_jax_array(x):
        from mdescriptor.descriptors.model_backed._vendor.dpa4desc.jax.env import (
            jax,
        )
...
def xp_uniform(like: Array, size: int, low: float = 0.0, high: float = 1.0) -> Array:
    ...
    from mdescriptor.descriptors.model_backed._vendor.dpa4desc.utils import random as dp_random
