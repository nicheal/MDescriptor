...
def _descriptor_state(raw_state: Mapping[str, Any]) -> dict[str, Tensor]:
    ...
    for key, value in state.items():
        ...
        if marker in key:
            ...
            if not isinstance(value, torch.Tensor):
                raise ValueError(f"DPA4C tensor {name!r} is not a tensor")
            result[name] = value
    if not result:
    ...
    if missing:
        raise ValueError(f"DPA4C checkpoint is missing tensor(s): {', '.join(missing)}")
    for name, value in result.items():
        if value.ndim == 0 or value.numel() == 0:
            raise ValueError(f"DPA4C tensor {name!r} must be non-empty")
        if name.endswith("_index") or name.endswith("mask"):
            integer_dtypes = {
                torch.int8, torch.int16, torch.int32, torch.int64,
                torch.uint8, torch.bool,
            }
            if not value.dtype.is_floating_point and value.dtype not in integer_dtypes:
                raise ValueError(f"DPA4C categorical tensor {name!r} has unsupported dtype")
        elif not value.dtype.is_floating_point:
            raise ValueError(f"DPA4C tensor {name!r} must use a floating dtype")
        if value.dtype.is_floating_point and not torch.isfinite(value).all():
            raise ValueError(f"DPA4C tensor {name!r} contains non-finite values")
    return result


def _validate_state_contract(state: dict[str, Tensor], config: DPA4CConfig) -> None:
    """Validate dtype and shape invariants before constructing the model."""

    compute_dtype = _torch_precision(config.precision)
    for name, value in state.items():
        ...
        if expected_dtype is not None and value.dtype != expected_dtype:
            raise ValueError(
                f"DPA4C tensor {name!r} has dtype {value.dtype}, expected {expected_dtype}"
            )
        if expected_dtype is None and value.dtype not in {
            torch.bool, torch.uint8, torch.int8, torch.int16, torch.int32, torch.int64
        }:
            raise ValueError(f"DPA4C mask tensor {name!r} has unsupported dtype {value.dtype}")

    type_embedding = state["type_embedding"]
    if type_embedding.ndim != 2 or type_embedding.shape[1] != config.channels:
        raise ValueError(
            "DPA4C type_embedding shape is inconsistent with checkpoint channels"
        )
    mean = state["mean"]
    stddev = state["stddev"]
    if mean.ndim != 1 or stddev.shape != mean.shape:
        raise ValueError("DPA4C calibration mean/stddev must be matching one-dimensional tensors")


def parse_dpa4c_checkpoint(
    raw: Mapping[str, Any],
) -> tuple[DPA4CConfig, dict[str, Tensor]]:
    """Validate a CPU checkpoint without constructing a device runtime."""

    if not isinstance(raw, Mapping):
        raise ValueError("DPA4C checkpoint must contain a mapping")
    state = _descriptor_state(raw)
    model_state = raw.get("model", raw)
    extra = model_state.get("_extra_state") if isinstance(model_state, Mapping) else None
    model_params = extra.get("model_params") if isinstance(extra, Mapping) else None
    if not isinstance(model_params, Mapping):
        raise ValueError("DPA4C checkpoint is missing model parameters")
    descriptor = model_params.get("descriptor")
    if not isinstance(descriptor, Mapping) or descriptor.get("type") != "dpa4c":
        raise ValueError("checkpoint descriptor.type is not 'dpa4c'")
    type_map = tuple(str(value).strip() for value in model_params.get("type_map", ()))
    if not type_map:
        raise ValueError("DPA4C checkpoint must contain a non-empty type_map")
    if any(not value for value in type_map) or len(set(type_map)) != len(type_map):
        raise ValueError("DPA4C checkpoint type_map must contain unique non-empty symbols")
    config = DPA4CConfig(
    ...
    if state["type_embedding"].shape[0] not in {config.ntypes, config.ntypes + 1}:
        raise ValueError(
            "DPA4C type_embedding shape is inconsistent with checkpoint type_map"
        )
    return config, state


def validate_dpa4c_checkpoint(checkpoint: Mapping[str, Any]) -> DPA4CConfig:
    """Validate a CPU checkpoint before it enters the shared model cache."""

    config, _ = parse_dpa4c_checkpoint(checkpoint)
    return config


def load_dpa4c_checkpoint(
    path: str,
    *,
    device: torch.device | str = "cpu",
    checkpoint: Mapping[str, Any] | None = None,
) -> tuple[DPA4CModel, DPA4CConfig]:
    ...
    if not isinstance(raw, Mapping):
        raise ValueError("DPA4C checkpoint must contain a mapping")
    config, state = parse_dpa4c_checkpoint(raw)
...
__all__ = [
    "DPA4CConfig",
    "DPA4CModel",
    "load_dpa4c_checkpoint",
    "parse_dpa4c_checkpoint",
