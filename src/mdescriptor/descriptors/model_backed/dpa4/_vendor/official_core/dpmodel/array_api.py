...
def xp_scatter_sum(input: Array, dim: int, index: Array, src: Array) -> Array:
    ...
    if getattr(xp, "__name__", "") == "upstream._vendors.ndtensorflow":
...
def xp_add_at(x: Array, indices: Array, values: Array) -> Array:
    ...
    elif getattr(xp, "__name__", "") == "upstream._vendors.ndtensorflow":
        import tensorflow as tf
...
def xp_uniform(like: Array, size: int, low: float = 0.0, high: float = 1.0) -> Array:
    ...
    other backends use :mod:`mdescriptor.descriptors.model_backed.dpa4._vendor.official_core.utils.random`. Draws are therefore not
    ...
    from mdescriptor.descriptors.model_backed.dpa4._vendor.official_core.utils import random as dp_random
