...
if TYPE_CHECKING:
    from mdescriptor.descriptors.model_backed.dpa4._vendor.official_core.dpmodel.array_api import Array

# The official native backend uses float64 as its global NumPy precision. Keep
# these names local so this port has no dependency on an upstream environment
# module.
GLOBAL_ENER_FLOAT_PRECISION = "float64"
...
PRECISION_DICT = {
    ...
    # NumPy has no bfloat16 dtype. The PyTorch path uses its native dtype and
    # the NumPy path only needs a stable host representation for constants.
    "bfloat16": np.float32,
}
...
def get_xp_precision(
    ...
    if precision == "float16" or precision == "half":
        ...
        return getattr(xp, "bfloat16", xp.float32)
    else:
