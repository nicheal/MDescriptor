...
class SoapKernel:
    ...
    def _cuda_payload(self) -> dict[str, Any]:
        """Return the small, private basis payload consumed by the CUDA path.

        The public configuration intentionally contains only the descriptor
        controls.  The CUDA implementation still needs the normalized radial
        basis generated by the CPU validation kernel, so keep those arrays on
        the private backend seam instead of rebuilding them in C++ with a
        second Python-facing convention.
        """

        if self.rbf == "gto":
            alphas, betas = _basis_gto(self.r_cut, self.n_max, self.l_max)
            radial = {"alphas": alphas, "betas": betas}
        else:
            grid, weights, values = _basis_polynomial(self.r_cut, self.n_max)
            radial = {
                "radial_grid": grid,
