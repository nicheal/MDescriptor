...
from .core import DescriptorResult, StructureBatch, _as_batch, _merge_config, _build_neighbor_graph
...
class Dpa4cCalculator:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        try:
            import torch
            from .._vendor.dpa4c.model import load_dpa4c_checkpoint
    ...
    def _validate_overrides(self, config: dict[str, Any]) -> None:
        for key in (
            ...
            if actual != expected:
                raise DescriptorConfigError(f"DPA4C config {key!r}={expected!r} disagrees with checkpoint value {actual!r}")
